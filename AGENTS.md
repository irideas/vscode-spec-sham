...
## 文档模板与结构基线
- **Index 页模板**：按顺序包含 (1) 引言/基线（域范围、版本基线）；(2) 角色与阅读路径；(3) 架构/生命周期摘要（3~6 条要点）；(4) 文档矩阵（四列表：域 | SRS | SDD | 摘要，链接用绝对路径 `/...`）；(5) 用例导航（列出用例 ID 与锚点链接）；(6) 质量与实践基线（性能/安全/可用性等）；(7) 怎么用（读者场景）。新增域的 `index.md` 必须对齐此序。
- **SRS 结构模板**：推荐章节顺序为 引言（目的/范围/读者/非目标） → 引用或基线 → 总体描述（角色、模型、系统概览、主要流程） → 外部接口与集成约束 → 功能性需求（按主题分节） → 质量/性能/安全/兼容性 → 用例（带 ID 与锚点） → 未来演进/占位。只写可验证事实，设计建议不要放入 SRS。
- **SDD 结构模板**：推荐章节顺序为 文档背景与目标 → 设计原则 → 使用者关注点/典型场景 → 架构视图（分层/数据流/运行时依赖） → 模块设计 → 模式示例（代码或流程） → 性能/安全/可观测性/测试建议 → 验证与演进路线。SDD 可以包含模式与反模式，但需与 SRS 事实区分。
